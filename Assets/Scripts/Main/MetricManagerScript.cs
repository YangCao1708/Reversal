...
public class MetricManagerScript : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (ShouldCollectData) //Collect on build, only collect on Editor if _collectDataOnEditor is true. This is to avoid trash files
        {
            ...
            _fileName = "Reversal_Metrics_" + _timeStamp + ".txt";

            try
            {
                _fileStreamWriter = File.CreateText("./" + _fileName);
                //_fileStreamWriter = File.CreateText("./Assets/Metrics/" + _fileName);//e.g. for the path something like /Users/UserName/Documents/PlayetestFiles/ will do
            }
            catch
            {
                Debug.Log("Could not create telemetry file " + "./" + _fileName);
    ...
    public void LogString(string variableName, string dataToLog)
    {
        if (ShouldCollectData)
        {
            ...
            string dataLine = variableName + " at game time " + minutesDisplay.ToString() + ":" + secondsDisplay.ToString() + " -- " + dataToLog.ToString() + Environment.NewLine;
